...
These are **things built on top of the kernel** (`../kernel`). They are
...
agent in ~20 lines — see [`../kernel/examples/agent.ts`](../kernel/examples/agent.ts)
and the [SDK guide](../kernel/SDK.md).

| Example | What it is |
|---------|------------|
| [`../kernel/examples/agent.ts`](../kernel/examples/agent.ts) | **Build your own agent** — the kernel SDK in ~20 lines (`createKernel` + `defineTool` + `run`). The best starting point. |
