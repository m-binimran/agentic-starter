 ...
 * Clicking it — or pressing Alt+J, or saying "Hey Agentic Starter" — opens the full
 * Agentic Starter app window, which loads the polished frontend (served on :3020) with
...
export function App() {
  ...
  const [thinking, setThinking] = useState(false) // Agentic Starter is processing — loading dots
  const openRef = useRef(false)

  // Poll the daemon's "busy" flag so the pill shows a loading animation whenever
  // Agentic Starter is thinking (a chat reply or a screen-guidance vision call in flight).
  useEffect(() => {
    let alive = true
    const tick = async () => {
      try {
  ...
  const apply = (next: boolean) => {
    ...
    setOpen(next)
    if (next) coordinator.expandSidebar()
    else coordinator.collapseBubble()
  }
  const toggle = () => apply(!openRef.current)

  // Alt+J relayed from the main process
  useEffect(() => coordinator.onToggleSidebar(() => toggle()), [])

  // "Hey Agentic Starter" wake word → open the app (non-blocking; ignored if mic denied)
  useEffect(() => {
